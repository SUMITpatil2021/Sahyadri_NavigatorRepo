...
public interface TrekkDetailsRepository extends JpaRepository<TrekkDetails, Integer> {
	...
	@Query(value="select t.trekkid, t.trekkname, t.charges,t.date ,t.clubid,"
			...
			+ " join image i "
			+ "on i.imgid=t.imgid where t.clubid = :cid",nativeQuery=true)
	public List<TrekkDetails> findTrekkDetails(int cid);
	...
	@Query(value="select t.trekkid, t.trekkname, t.charges,t.date ,t.clubid,"
			...
			+ " on f.facilityid=t.facilityid"
			+ " join image i "
			+ "on i.imgid=t.imgid where t.date>CURRENT_DATE ",nativeQuery=true)
